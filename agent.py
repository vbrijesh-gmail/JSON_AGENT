...
async def run_json_task(prompt: str, json_obj: Dict[str, Any]) -> Dict[str, Any]:
    """Accepts a natural language prompt and a JSON object, and returns the modified JSON.

    Strategy:
    - Save the provided JSON to a temp file.
    - Ask the agent to read the file, apply the requested changes, and write back.
    - Read the file and return its contents as the updated JSON.
    """
